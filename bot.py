...
# Check Variables
if not all([BOT_TOKEN, API_ID, API_HASH, TMDB_API_KEY, MONGO_URL, DB_CHANNEL_ID]):
...
@app.route('/')
def home():
    return "🤖 Fast Telegram SPA Bot Running"
...
# ============================================================================
# 🔥 ADVANCED HTML GENERATOR (TELEGRAM ONLY & 18+ BLUR EFFECT)
# ============================================================================
def generate_html_code(data, links, user_ad_links_list, owner_ad_links_list, admin_share_percent=20):
    ...
    if is_adult:
        poster_html = f'''<div class="nsfw-container" onclick="revealNSFW(this)"><img src="{poster}" alt="{title} Poster" class="nsfw-blur"><div class="nsfw-warning">🔞 18+<br><small style="font-size:10px;">Click to Reveal</small></div></div>'''
    else:
    ...
    trailer_html = ""
    if trailer_key:
        trailer_html = f'''<div class="section-title">🎬 Official Trailer</div><div class="video-container"><iframe src="https://www.youtube.com/embed/{trailer_key}" allowfullscreen></iframe></div>'''
    ...
    # 🔥 GENERATE TELEGRAM DOWNLOAD LINKS (GROUPED BY QUALITY/EPISODE) 🔥
    server_list_html = ""
    grouped_links = {}
    for link in links:
    ...
    for lbl, grp in grouped_links.items():
        ...
        for link in grp:
            tg_url = link.get('tg_url', '')
            if tg_url:
                encoded_url = base64.b64encode(tg_url.encode('utf-8')).decode('utf-8')
                server_list_html += f'<button class="final-server-btn tg-btn" onclick="goToLink(\'{encoded_url}\')">✈️ Download via Telegram</button>'
    ...
    <style>
        ...
        .final-server-btn {{ width: 100%; padding: 14px; font-size: 15px; font-weight: bold; color: #fff; border: none; border-radius: 6px; cursor: pointer; transition: 0.2s; box-shadow: 0 4px 10px rgba(0,0,0,0.3); }}
    ...
    <div class="app-wrapper">
        <div id="view-details">
            ...
            <div style="background: rgba(0,0,0,0.1); padding: 12px; border-radius: 6px; font-size: 13px; text-align: center; margin-bottom: 15px; color: var(--text-muted); border: 1px solid var(--border);">
                ℹ️ <b>How to Download:</b> Click the button below, wait 5 seconds, and your download links will unlock.
            </div>
            
            <div class="action-grid">
                <button class="main-btn btn-download" onclick="startUnlock(this)">
        ...
        <div id="view-links">
            ...
            <p style="font-size: 14px; color: var(--text-muted); margin-bottom: 15px;">Please select your desired quality or episode below to download instantly.</p>
...
async def my_settings_cmd(client, message):
    ...
    text = f"⚙️ **Your Settings**\n\n👤 **Name:** {message.from_user.first_name}\n🆔 **ID:** `{uid}`\n\n📢 **Your Ad Links:**\n{ads_text}\n\n💡 _Use /setadlink to update your ads._"
...
# 🔥 TELEGRAM DIRECT DATABASE UPLOAD (SUPER FAST)
async def process_file_upload(client, message, uid, temp_name):
    convo = user_conversations.get(uid)
    if not convo: return
    
    convo["pending_uploads"] = convo.get("pending_uploads", 0) + 1
    status_msg = await message.reply_text(f"⏳ **সেভ হচ্ছে...** ({temp_name})", quote=True)
    
    try:
        async with upload_semaphore:
            # File is NOT downloaded. Just copied to DB channel. Lightning fast.
...
@bot.on_message(filters.private & (filters.text | filters.video | filters.document | filters.photo) & ~filters.command(["start", "post", "manual", "edit", "history", "setadlink", "mysettings", "auth", "ban", "stats", "broadcast", "setownerads", "setshare", "setdel", "cancel"]))
...
if __name__ == "__main__":
    ...
    print("🚀 Fast SPA Bot is Starting with Plugin System...")
