...
def get_premium_css(theme):
    ...
    if theme == "prime":
        return base_style + "<style>.info-poster img{box-shadow: 0 10px 30px rgba(0, 168, 225, 0.4) !important;} .info-text span, .quality-title, .final-server-btn:hover{background:#00A8E1 !important; border-color:#00A8E1 !important; color:#fff !important;}</style>"
    elif theme == "light": # এটি এখন 'Glow Pink' থিম হবে
        return base_style + "<style>.info-poster img{box-shadow: 0 10px 30px rgba(255, 121, 198, 0.4) !important;} .info-text span, .quality-title, .final-server-btn:hover{background:#ff79c6 !important; border-color:#ff79c6 !important; color:#fff !important;}</style>"
...
# ⚠️ Safety Guard: Prevent infinite recursion and multiple patching
if not hasattr(__main__, "_premium_template_patched"):
    ...
    def premium_html_generator(data, links, user_ads, owner_ads, share):
        # বেসিক HTML নেওয়া (মেইন কোড বা অন্য প্লাগইন থেকে)
        ...
        schema = {
            ...
            "name": title,
            "image": poster,
            "description": data.get("overview", "Download and watch latest movies and series.")[:160]
        }
        schema_code = f'<script type="application/ld+json">\n{json.dumps(schema, indent=2)}\n</script>'
        
        # মেইন HTML এর শুরুতে প্রিমিয়াম সিএসএস এবং স্কিমা ইনজেক্ট করা
        return f"{schema_code}\n{premium_css}\n{html}"

    # মেইন স্ক্রিপ্টের জেনারেটর রিপ্লেস করা
    __main__.generate_html_code = premium_html_generator
    __main__._premium_template_patched = True # ফ্ল্যাগ সেট করা হলো

async def register(bot):
    print("💎 Premium Templates (Advanced UX): Activated Successfully!")
